...
export class FileUploaderComponent extends Base
  ...
  @Output() imagesChange: EventEmitter<Array<CommonImage>> = new EventEmitter<
    Array<CommonImage>
  >();
  @Input() id: string;
  ...
  @Output() errorDocument: EventEmitter<CommonImage> = new EventEmitter<
    CommonImage
  >();
  ...
  ngOnInit(): void {
    const dragOverStream = fromEvent<DragEvent>(
    ...
    const dropStream = fromEvent<DragEvent>(
      this.dropZone.nativeElement,
      'drop'
    );
    ...
    const browseFileStream = fromEvent<Event>(
  ...
  observableFromFiles(
    ...
    // Init
    const self = this;
    /* let  pageNumber = Math.max(...self.images.concat( self.application.getAllImages()).map(function(o) {
      return o.attachmentOrder;
    }), 0) + 1 ;*/
    let pageNumber =
      Math.max(
        ...self.images.map(function (o) {
          return o.attachmentOrder;
        }),
        0
      ) + 1;
    ...
    const fileObservable = new Observable((observer: Observer<CommonImage>) => {
      ...
      for (let fileIndex = 0; fileIndex < fileList.length; fileIndex++) {
        ...
        if (file.type === 'application/pdf') {
          ...
          /**
           *  Page number logic :
           *      Images - Assign current page number whichever is available..so get the current page number,
           *      pass it to call back [reserve it] and increment
           *      PDF    -  we dont know how many pages..so cant get current number and keep it since it can
           *      be multiple pages... so start assigning later point
          ...
          this.readPDF(
            file,
            pdfScaleFactor,
            (images: HTMLImageElement[], pdfFile: File) => {
  ...
  checkImageDimensions(file: CommonImage): boolean {
    if (file.naturalHeight < 0 || file.naturalWidth < 0) {
      return false;
    }
    return true;
  }
